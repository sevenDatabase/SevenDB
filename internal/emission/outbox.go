...
func (o *outboxState) pendingSorted(sub string) []*OutboxEntry {
    ...
    for k := range m { idxs = append(idxs, int(k)) }
    sort.Ints(idxs)
    out := make([]*OutboxEntry, 0, len(m))
    for _, i := range idxs { out = append(out, m[uint64(i)]) }
    return out
}
...
func NewManager() *Manager { return &Manager{ob: newOutboxState(), lastAck: make(map[string]uint64), compactThrough: make(map[string]uint64)} }
