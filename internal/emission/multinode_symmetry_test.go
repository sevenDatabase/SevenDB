...
type localMemTransport struct {
	nodes map[uint64]*raft.ShardRaftNode
}

func newLocalMemTransport() *localMemTransport {
	return &localMemTransport{nodes: map[uint64]*raft.ShardRaftNode{}}
}
	...
	for attempts := 0; attempts < 10; attempts++ {
		...
		if _, _, err := ln.ProposeAndWait(ctx, rec); err != nil {
			...
			if err == context.DeadlineExceeded {
				t.Fatalf("proposal timeout: %v", err)
			}
...
func TestEmission_MultiReplicaSymmetry_3Nodes(t *testing.T) {
	...
	if got < N {
		t.Fatalf("leader delivered %d events, want %d", got, N)
	}
	...
	for i, n := range nodes {
		if n == ln {
			h1 = canonicalHash(senders[i].Snapshot(), N)
		}
	}
	...
	if ln2 == nil {
		t.Fatalf("no leader in second cluster")
	}
	for i, n := range nodes {
		if n == ln2 {
			notifiers[i].SetSender(senders[i])
	...
	if got < N {
		t.Fatalf("leader delivered (2) %d, want %d", got, N)
	}
	for i, n := range nodes {
		if n == ln2 {
			h2 = canonicalHash(senders[i].Snapshot(), N)
		}
	}

	if h1 != h2 {
		t.Fatalf("canonical emission hash mismatch across clusters: %s vs %s", h1, h2)
	}
}
