...
func TestRaftBucketLog_AppendRead(t *testing.T) {
	...
	for i, b := range buckets {
		...
		if ci == 0 {
			t.Fatalf("expected non-zero commit index")
		}
	}
...
loop:
	...
	if len(seenA) != 3 {
		t.Fatalf("expected 3 entries for A, got %d", len(seenA))
	}
	for i := 1; i <= 3; i++ {
		if seenA[i-1] != uint64(i) {
			t.Fatalf("expected commit index %d at position %d, got %d", i, i-1, seenA[i-1])
		}
	}
}
