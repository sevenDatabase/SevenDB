...
func TestFileBucketLog_EpochAndOffsetMonotonic(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("new file bucket log: %v", err)
	}
	defer fbl.Close()
	b := BucketID("epoch1")
	...
	for i := 0; i < 5; i++ {
		...
		if ci != uint64(i+1) {
			t.Fatalf("commit index expected %d got %d", i+1, ci)
		}
		if off < lastOffset {
			t.Fatalf("file offset not monotonic: %d < %d", off, lastOffset)
		}
		lastOffset = off
	}
	...
	for e := range ch {
		if e.Epoch.Counter != 0 {
			t.Fatalf("unexpected epoch counter %d", e.Epoch.Counter)
		}
		if e.Epoch.Bucket != b {
			t.Fatalf("epoch bucket mismatch %s != %s", e.Epoch.Bucket, b)
		}
	}
}
...
func TestBucket_RunExported(t *testing.T) {
	...
	go b.Run(ctx)
	for i := 0; i < 3; i++ {
		_, _, _ = fbl.Append(ctx, &WALEntry{BucketID: b.ID, Type: RecDataUpdate})
	}
	time.Sleep(100 * time.Millisecond)
	if b.lastApplied != 3 {
		t.Fatalf("expected lastApplied=3 got %d", b.lastApplied)
	}
	cancel()
