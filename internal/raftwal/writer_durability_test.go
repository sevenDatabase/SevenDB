...
func TestStrictSyncDurability(t *testing.T) {
	...
	for i := 1; i <= 10; i++ {
		...
		if err := w.AppendEnvelope(uint64(i), b); err != nil { t.Fatalf("append: %v", err) }
	}
	...
	if err := w.AppendHardState(10, hb); err != nil { t.Fatalf("append hs: %v", err) }
	...
	if _, err := os.Stat(seg0+".idx"); err == nil { _ = os.Remove(seg0+".idx") }

	// Replay and check we recovered all 10 normal entries and one hardstate
	var count int
	var lastHS bool
	if err := Replay(dir, func(env *Envelope) error {
		if env.Kind == EntryKind_ENTRY_NORMAL { count++ }
		if env.Kind == EntryKind_ENTRY_HARDSTATE { lastHS = true }
		return nil
	}); err != nil { t.Fatalf("replay: %v", err) }
	if count != 10 { t.Fatalf("expected 10 normal entries, got %d", count) }
	if !lastHS { t.Fatalf("expected hardstate envelope present") }
}
