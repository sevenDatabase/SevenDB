...
func TestWriterCrashInjection(t *testing.T) {
	...
	for _, p := range points {
		t.Run(p, func(t *testing.T) {
			...
			switch p {
			case "afterFrame":
				w.injAfterFrameWrite = func(kind string, idx uint64) {
					if idx == 5 { /* crash */
						w = nil
					}
				}
			case "afterFlush":
				w.injAfterFlushFsync = func(kind string, idx uint64) {
					if idx == 5 {
						w = nil
					}
				}
			...
			for i := uint64(1); i <= uint64(len(seen)); i++ {
				if !seen[i] {
					t.Fatalf("missing index %d for crash point %s", i, p)
				}
			}
		})
	}
...
func TestSidecarLossEnriched(t *testing.T) {
	...
	for i := uint64(1); i <= 6; i++ {
		b, _ := proto.Marshal(buildTestEnvelope(i))
		if err := w.AppendEnvelope(i, b); err != nil {
	...
	if count != 6 {
		t.Fatalf("expected 6 envelopes, got %d", count)
	}
}
