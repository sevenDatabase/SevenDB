...
func TestWALPruneAfterSnapshot(t *testing.T) {
	...
	defer n.Close()
	for i := 0; i < 300 && !n.IsLeader(); i++ {
		advanceAll([]*ShardRaftNode{n}, 10*time.Millisecond)
	}
	if !n.IsLeader() {
	...
	if fw, ok := n.walShadow.(forceSetter); ok {
		fw.SetForceRotateEvery(5)
	}
...
func TestWALPruneNoEligible(t *testing.T) {
	...
	if len(leftover) != 0 {
		t.Fatalf("unexpected deleted markers: %v", leftover)
	}
}
...
func TestWALPruneRecoveryDeletedCleanup(t *testing.T) {
	...
	f, _ := os.Create(filepath.Join(walDir, "seg-0.wal.deleted"))
	f.Close()
	f2, _ := os.Create(filepath.Join(walDir, "seg-0.wal.idx.deleted"))
	f2.Close()
	cfg := RaftConfig{ShardID: shardID, NodeID: "1", Engine: "etcd", DataDir: root, ForwardProposals: true,
	...
	if len(leftover) != 0 {
		t.Fatalf("deleted markers not cleaned: %v", leftover)
	}
}
