...
func waitApplied(t *testing.T, nodes []*ShardRaftNode, target uint64) {
	waitUntil(t, 10*time.Second, 50*time.Millisecond, func() bool {
		for _, n := range nodes {
			if n.Status().LastAppliedIndex < target {
				return false
			}
		}
		return true
...
func TestRaftClusterBasicReplicationAndSnapshot(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		if cIdx <= lastCommit {
			t.Fatalf("commit index not increasing: %d <= %d", cIdx, lastCommit)
		}
		if rIdx <= lastRaft {
			t.Fatalf("raft index not increasing: %d <= %d", rIdx, lastRaft)
		}
	...
	for i := 0; i < 1000; i++ { // deterministic apply wait
		all := true
		for _, n := range nodes {
			if n.Status().LastAppliedIndex < lastRaft {
				all = false
				break
			}
		}
	...
	for _, n := range nodes {
		if n.Status().LastAppliedIndex < lastRaft {
			t.Fatalf("apply lag after timeout")
		}
	}
	...
	for i := 0; i < 1000; i++ {
		snap := false
		for _, n := range nodes {
			if n.Status().LastSnapshotIndex > 0 {
				snap = true
				break
			}
		}
	...
	if !hasSnap {
		t.Fatalf("snapshot not triggered in deterministic window")
	}
	...
	for _, n := range nodes {
		...
		cancel()
		if err == nil {
			t.Fatalf("expected NotLeaderError from follower")
		}
		var nle *NotLeaderError
		if !errors.As(err, &nle) {
			t.Fatalf("expected NotLeaderError, got %v", err)
		}
		if nle.LeaderID != leaderID {
			t.Fatalf("leader hint mismatch: %s vs %s", nle.LeaderID, leaderID)
		}
		break
	}
	...
	st := newNode.Status()
	if st.LastAppliedIndex < preApplied {
		t.Fatalf("restarted node applied index regressed: %d < %d", st.LastAppliedIndex, preApplied)
	}
	if preSnapshot > 0 && st.LastSnapshotIndex == 0 {
		t.Fatalf("snapshot index lost after restart: was %d now %d", preSnapshot, st.LastSnapshotIndex)
	}
...
func TestRaftLogPruneAfterSnapshot(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		...
		if nodes[0].Status().LastSnapshotIndex > 0 {
			break
		}
	}
	snapIdx := nodes[0].Status().LastSnapshotIndex
	if snapIdx == 0 {
		t.Fatalf("node-1 never produced snapshot within proposal budget")
	}
	// Wait for pruning completion signaled by prunedThroughIndex >= snapshot index.
	for i := 0; i < 1000; i++ {
		st := nodes[0].Status()
		if st.PrunedThroughIndex >= snapIdx {
			break
		}
		advanceAll(nodes, 10*time.Millisecond)
	}
	if nodes[0].Status().PrunedThroughIndex < snapIdx {
		t.Fatalf("prune did not reach snapshot index")
	}
}
...
func TestRaftFollowerRestartCatchUp(t *testing.T) {
	...
	var last uint64
	for i := 0; i < 5; i++ {
		_, rIdx := proposeOnLeader(t, nodes, "bx", []byte("seed"))
		last = rIdx
	}
	for i := 0; i < 1000; i++ {
		all := true
		for _, n := range nodes {
			if n.Status().LastAppliedIndex < last {
				all = false
				break
			}
		}
	...
	for i := 0; i < 6; i++ {
		_, rIdx := proposeOnLeader(t, activeNodes, "bx", []byte("later"))
		last = rIdx
	}
	...
	for i := 0; i < 1000; i++ {
		...
		advanceAll(activeNodes, 10*time.Millisecond)
	}
	for _, n := range activeNodes {
		if n.Status().LastAppliedIndex < last {
			t.Fatalf("apply lag active cluster")
		}
	}
	// Restart follower
	...
	mtrans.attach(uint64(followerIdx+1), newFollower)
	newFollower.SetTransport(mtrans)
	...
	for _, n := range nodes {
		if n.Status().LastAppliedIndex < last {
			t.Fatalf("apply lag after follower restart")
		}
	}
	if newFollower.Status().LastAppliedIndex <= preIdx {
		t.Fatalf("follower didn't advance after restart: %d <= %d", newFollower.Status().LastAppliedIndex, preIdx)
	}
}
...
func TestRaftMultiBucketCommitIsolation(t *testing.T) {
	...
	validate := func(name string, s []uint64) {
		...
		for i, v := range sorted {
			expect := uint64(i + 1)
			if v != expect {
				t.Fatalf("%s commit index gap: got %v at pos %d expect %d (raw=%v)", name, v, i, expect, copyS)
			}
		}
	}
...
func TestNotLeaderErrorSemantics(t *testing.T) {
	...
	var nle *NotLeaderError
	if !errors.As(err, &nle) {
		t.Fatalf("expected NotLeaderError type got %v", err)
	}
	if nle.LeaderID != leaderID {
		t.Fatalf("leader id mismatch hint=%s actual=%s", nle.LeaderID, leaderID)
	}
}
