...
func TestForgeWALPrimaryReadSeedsStorage(t *testing.T) {
    ...
    if s, ok := wPkg.DefaultWAL.(interface{ Sync() error }); ok { _ = s.Sync() }
    ...
    if err != nil { t.Fatalf("new shard node: %v", err) }
    t.Cleanup(func(){ _ = n.Close() })

    ents, eerr := n.storage.Entries(1, 4, 1<<20)
    if eerr != nil { t.Fatalf("entries: %v", eerr) }
    if len(ents) != 3 { t.Fatalf("expected 3 entries, got %d", len(ents)) }
    for i, e := range ents {
        if e.Index != uint64(i+1) { t.Fatalf("entry index mismatch at %d: %d", i, e.Index) }
        if e.Term != 1 { t.Fatalf("entry term mismatch at %d: %d", i, e.Term) }
        if len(e.Data) != 1 || e.Data[0] != byte('a'+i) { t.Fatalf("entry data mismatch at %d: %v", i, e.Data) }
    }
}
