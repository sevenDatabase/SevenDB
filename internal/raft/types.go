...
func NewShardRaftNode(cfg RaftConfig) (*ShardRaftNode, error) {
	...
	if engine == "etcd" {
		if err := s.initEtcd(cfg); err != nil { return nil, err }
	}
	return s, nil
}
...
func (s *ShardRaftNode) ProposeAndWait(ctx context.Context, rec *RaftLogRecord) (uint64, uint64, error) {
	...
	if s.engine == "etcd" && s.rn != nil {
		...
		if err = s.rn.Propose(ctx, data); err != nil { return 0,0, err }
		select {
		case <-ctx.Done():
			return 0,0, ctx.Err()
		case res := <-ch:
...
func (s *ShardRaftNode) Close() error {
	...
	for _, ch := range waitersSeq { close(ch) }
...
func (s *ShardRaftNode) initEtcd(cfg RaftConfig) error {
	var id uint64 = 1
	if cfg.NodeID != "" { if v, err := strconv.ParseUint(cfg.NodeID,10,64); err==nil { id = v } }
	hb := cfg.HeartbeatMillis; if hb <= 0 { hb = 100 }
	et := cfg.ElectionTimeoutMillis; if et <= 0 { et = 1000 }
	electionTicks := et / hb; if electionTicks <= 1 { electionTicks = 5 }
	heartbeatTicks := 1
	...
	s.rn = etcdraft.StartNode(c, []etcdraft.Peer{{ID: id}})
...
func (s *ShardRaftNode) readyLoop() {
	for {
		select { case <-s.stopCh: return; default: }
		rd, ok := <-s.rn.Ready(); if !ok { return }
...
func (l *etcdLoggerAdapter) Debug(args ...interface{})  { slog.Debug("etcdraft", slog.Any("args", args)) }
...
func (l *etcdLoggerAdapter) Error(args ...interface{})  { slog.Error("etcdraft", slog.Any("args", args)) }
...
func (l *etcdLoggerAdapter) Debugf(format string, args ...interface{}) { slog.Debug("etcdraft", slog.String("msg", fmt.Sprintf(format, args...))) }
func (l *etcdLoggerAdapter) Infof(format string, args ...interface{})  { slog.Info("etcdraft", slog.String("msg", fmt.Sprintf(format, args...))) }
func (l *etcdLoggerAdapter) Warnf(format string, args ...interface{})  { slog.Warn("etcdraft", slog.String("msg", fmt.Sprintf(format, args...))) }
func (l *etcdLoggerAdapter) Errorf(format string, args ...interface{}) { slog.Error("etcdraft", slog.String("msg", fmt.Sprintf(format, args...))) }
func (l *etcdLoggerAdapter) Fatal(args ...interface{})                 { slog.Error("etcdraft", slog.Any("fatal", args)) }
func (l *etcdLoggerAdapter) Fatalf(format string, args ...interface{}) { slog.Error("etcdraft", slog.String("fatal", fmt.Sprintf(format, args...))) }
func (l *etcdLoggerAdapter) Panic(args ...interface{})                 { slog.Error("etcdraft", slog.Any("panic", args)) }
func (l *etcdLoggerAdapter) Panicf(format string, args ...interface{}) { slog.Error("etcdraft", slog.String("panic", fmt.Sprintf(format, args...))) }
