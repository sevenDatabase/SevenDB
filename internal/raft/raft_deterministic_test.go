...
func TestDeterministicManualMode(t *testing.T) {
	...
	if !n.IsLeader() {
		n.ManualForceCampaign()
	}
	for i := 0; i < 50 && !n.IsLeader(); i++ {
		n.ManualTick()
		for n.ManualProcessReady() {
		}
	}
	...
	for i := 0; i < proposals; i++ {
		if _, err := n.Propose(ctx, &RaftLogRecord{BucketID: bucket, Type: 1, Payload: []byte(strconv.Itoa(i))}); err != nil {
		...
		drain:
			for {
				...
				case rec := <-n.Committed():
					if rec != nil && rec.Record != nil && rec.Record.BucketID == bucket {
						committed++
					}
				default:
					break drain
	...
	if committed != proposals {
		t.Fatalf("unexpected committed=%d want=%d", committed, proposals)
	}
}
