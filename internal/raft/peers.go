...
func ParsePeerSpecs(specs []string) (map[uint64]string, []string, error) {
    ...
    for _, s := range specs {
        ...
        if at := strings.IndexByte(s, '@'); at != -1 {
            ...
            if err != nil { return nil, nil, fmt.Errorf("invalid peer id '%s' in '%s'", idPart, s) }
            if addr == "" { return nil, nil, fmt.Errorf("empty addr in peer spec '%s'", s) }
            if _, exists := addrs[id]; exists { return nil, nil, fmt.Errorf("duplicate peer id %d", id) }
            addrs[id] = addr
            explicit[addr] = id
            used[id] = struct{}{}
        } else {
    ...
    for _, addr := range implicit {
        if _, dup := explicit[addr]; dup { continue }
        for {
            if _, taken := used[nextID]; !taken { break }
            nextID++
        }
        addrs[nextID] = addr
        used[nextID] = struct{}{}
    ...
    for id := range addrs { ids = append(ids, id) }
    sort.Slice(ids, func(i,j int) bool { return ids[i] < ids[j] })
    canonical := make([]string, 0, len(ids))
    for _, id := range ids { canonical = append(canonical, fmt.Sprintf("%d@%s", id, addrs[id])) }
...
func DetermineLocalID(cfgNodeID string, advertiseAddr string, peers map[uint64]string) (uint64, error) {
    if cfgNodeID != "" {
        ...
        if err != nil { return 0, fmt.Errorf("invalid raft-node-id '%s'", cfgNodeID) }
        if _, ok := peers[v]; ok { return v, nil }
        return 0, fmt.Errorf("raft-node-id %d not present in peer set", v)
    }
    if advertiseAddr != "" {
        for id, addr := range peers { if addr == advertiseAddr { return id, nil } }
    }
    if len(peers) == 1 {
        for id := range peers { return id, nil }
    }
