...
func TestWALPrimaryReadSeedsStorage(t *testing.T) {
	...
	for i := 1; i <= 3; i++ {
		...
		if err := w.AppendEnvelope(uint64(i), b); err != nil { t.Fatalf("append %d: %v", i, err) }
	}
	...
	if err := w.AppendHardState(3, hb); err != nil { t.Fatalf("append hs: %v", err) }
	_ = w.Close()
	...
	for i, e := range entries { if e.Index != uint64(i+1) { t.Fatalf("entry index mismatch at %d: %d", i, e.Index) } }
