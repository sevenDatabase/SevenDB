...
func (a *shadowWALAdapter) AppendEntry(index, term uint64, kind EntryKind, appBytes []byte, meta WALMetadata, rawHardState []byte) error {
	...
	case EntryKindHardState:
		...
		if len(rawHardState) > 0 {
			env.AppBytes = rawHardState
			env.AppCrc = crc32.ChecksumIEEE(rawHardState)
		}
	}
...
func (a *shadowWALAdapter) Replay(cb func(*WALReplayItem) error) error {
	...
	return raftwal.Replay(a.dir, func(env *raftwal.Envelope) error {
		...
		if env.Kind == raftwal.EntryKind_ENTRY_HARDSTATE {
			item.Kind = EntryKindHardState
		} else {
			item.Kind = EntryKindNormal
		}
		if item.Kind == EntryKindHardState {
			item.RawHardState = env.AppBytes
		}
		return cb(item)
	})
}
...
func (a *shadowWALAdapter) Dir() string {
	if a.w == nil {
		return a.dir
	}
	return a.w.Dir()
...
func (a *shadowWALAdapter) Close() error {
	if a.w == nil {
		return nil
	}
	return a.w.Close()
