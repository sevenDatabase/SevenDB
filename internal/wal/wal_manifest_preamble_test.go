...
func TestManifestPreambleWrittenAndMatchesHash(t *testing.T) {
	...
	if s, ok := DefaultWAL.(interface{ Sync() error }); ok {
		_ = s.Sync()
	}
	t.Cleanup(func() {
		if DefaultWAL != nil {
	...
	if el.ElementType != w.ElementType_ELEMENT_TYPE_COMMAND {
		t.Fatalf("unexpected element type: %v", el.ElementType)
	}
	if !isUWAL(el.Payload) {
		t.Fatalf("first payload not UWAL")
	}
	kind, _, _, _, inner, _, err := decodeUWAL(el.Payload)
	if err != nil {
		t.Fatalf("decode uwal: %v", err)
	}
	if kind != uwalKindManifest {
		t.Fatalf("first entry not manifest preamble: kind=%d", kind)
	}
	if string(inner) != string(sum[:]) {
		t.Fatalf("manifest hash mismatch: wal=%x want=%x", inner, sum[:])
	}
}
...
func TestManifestPreambleMismatchStrictFailsInit(t *testing.T) {
	...
	if err != nil || mf == nil {
		t.Fatalf("load manifest: %v, mf=%v", err, mf)
	}
	mf.Enforce = EnforceStrict
	mf.WriterVersion = mf.WriterVersion + "-changed"
	if _, err := SaveManifestAtomic(walDir, mf); err != nil {
...
func TestManifestPreambleSkippedInReplayAndLastIndex(t *testing.T) {
	...
	count := 0
	var last ReplayItem
	if err := uw.ReplayItems(func(ri ReplayItem) error { count++; last = ri; return nil }); err != nil {
		t.Fatalf("replay: %v", err)
	}
	if count < 1 {
		t.Fatalf("expected at least 1 replay item, got %d", count)
	}
	if last.Index != 42 || last.Term != 7 {
		t.Fatalf("unexpected last replay item: %+v", last)
	}

	// lastIndexOfSegment should ignore the preamble and return 42 for seg-0
	wf, ok := DefaultWAL.(*walForge)
	if !ok {
		t.Fatalf("default wal not *walForge")
	}
	segPath := filepath.Join(walDir, segmentPrefix+"0"+".wal")
	...
	if idx != 42 {
		t.Fatalf("last index mismatch: got %d want %d", idx, 42)
	}
}
