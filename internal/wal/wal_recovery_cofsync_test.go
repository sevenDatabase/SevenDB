...
func TestCrashAfterFlushBeforeFsync_NoHardStatePersisted(t *testing.T) {
    ...
    wf := newWalForge()
    if err := wf.Init(); err != nil { t.Fatalf("init: %v", err) }
    ...
    if wf.bufferSyncTicker != nil { wf.bufferSyncTicker.Stop() }
    if wf.segmentRotationTicker != nil { wf.segmentRotationTicker.Stop() }
    if wf.csf != nil { _ = wf.csf.Close() }
    if wf.cancel != nil { wf.cancel() }
    ...
    wf2 := newWalForge()
    if err := wf2.Init(); err != nil { t.Fatalf("reinit: %v", err) }
    defer wf2.Stop()
}
