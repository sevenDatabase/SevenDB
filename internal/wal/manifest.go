...
func SaveManifestAtomic(dir string, m *Manifest) ([]byte, error) {
    if m.CreatedAt.IsZero() { m.CreatedAt = time.Now().UTC() }
    ...
    if err := os.WriteFile(tmp, data, 0644); err != nil { return nil, err }
    f, err := os.Open(tmp)
    if err != nil { return nil, err }
    if err := f.Sync(); err != nil { _ = f.Close(); return nil, err }
    _ = f.Close()
    if err := os.Rename(tmp, target); err != nil { return nil, err }
    d, err := os.Open(dir)
    if err != nil { return nil, err }
    if err := d.Sync(); err != nil { _ = d.Close(); return nil, err }
    _ = d.Close()
...
func DetectFormat(dir string, maxSegments int, maxEntries int) (WALFormat, error) {
    ...
    if len(segments) == 0 { return WALFormatUWAL1, nil } // empty dir → future is UWAL1
    segLimit := len(segments)
    if maxSegments > 0 && maxSegments < segLimit { segLimit = maxSegments }
    uwalCount := 0
    legacyCount := 0
    entriesScanned := 0
    for i := 0; i < segLimit; i++ {
        ...
        for entriesScanned < maxEntries || maxEntries == 0 {
            ...
            entriesScanned++
            if maxEntries > 0 && entriesScanned >= maxEntries { break }
        }
        _ = f.Close()
        if maxEntries > 0 && entriesScanned >= maxEntries { break }
    }
    if uwalCount > 0 && legacyCount == 0 { return WALFormatUWAL1, nil }
    if uwalCount == 0 && legacyCount > 0 { return WALFormatLegacy, nil }
    if uwalCount == 0 && legacyCount == 0 { return WALFormatUWAL1, nil } // empty segments → treat as uwal1
...
func (m *Manifest) VerifyAgainst(detected WALFormat) error {
    ...
    if m.IsUWAL1() {
        ...
        case WALFormatUWAL1:
            return nil
        case WALFormatMixed:
            if m.Compat.MixedOK { return nil }
