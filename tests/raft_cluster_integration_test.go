...
func TestRaftClusterIntegrationBasic(t *testing.T) {
	...
	if runtime.GOOS != "linux" {
		t.Skip("integration cluster test currently validated on linux only")
	}
	...
	for i := range nodes {
		...
		if err := cmd.Start(); err != nil {
			t.Fatalf("start node %d: %v", nodes[i].id, err)
		}
	...
	for time.Now().Before(deadline) {
		...
		for i := range nodes {
			...
			if parsed.Nodes[0].IsLeader {
				leaders++
				leaderNode = &nodes[i]
			}
		}
		if leaderNode != nil && leaders == 1 {
			break
		}
		time.Sleep(200 * time.Millisecond)
	}
	if leaderNode == nil {
		dumpNodeLogs(t, nodes)
	...
	for i := range nodes {
		if nodes[i].id != leaderNode.id {
			follower = &nodes[i]
			break
		}
	}
	if follower != nil {
		_ = sendRESP(follower.clientPort, "GET", "itest:key")
	}
...
func readAppliedIndices(nodes []nodeSpec) []uint64 {
	...
	for i, n := range nodes {
		...
		if len(parsed.Nodes) > 0 {
			out[i] = parsed.Nodes[0].LastApplied
		}
	}
	return out
}
...
func dumpNodeLogs(t *testing.T, nodes []nodeSpec) {
	for _, n := range nodes {
		if n.stdout.Len() > 0 {
			t.Logf("[node%d stdout]\n%s", n.id, n.stdout.String())
		}
		if n.stderr.Len() > 0 {
			t.Logf("[node%d stderr]\n%s", n.id, n.stderr.String())
		}
		if n.cmd != nil {
			if st := n.cmd.ProcessState; st != nil {
				t.Logf("[node%d exit] %v", n.id, st)
			}
		}
		// Attempt inline status decode for final diagnostics
		if b, err := os.ReadFile(n.statusPath); err == nil {
			t.Logf("[node%d status.json]\n%s", n.id, string(b))
		} else {
			t.Logf("[node%d status.json missing] %v", n.id, err)
		}
	}
}
